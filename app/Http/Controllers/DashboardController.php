...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $result['info'] = DB::select("SELECT COUNT(id) AS total, SUM(CASE WHEN status = 'accepted' THEN 1 ELSE 0 END) AS accepted,SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END) AS active,SUM(CASE WHEN status = 'rejected' THEN 1 ELSE 0 END) AS rejected,SUM(CASE WHEN is_important = 1 THEN 1 ELSE 0 END) AS importants FROM `tasks` WHERE status != 'draft' AND user_id = '$user->id'");
