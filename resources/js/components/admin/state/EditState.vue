...
export default {
  ...
  computed: {
    ...mapGetters("mystate", ["getStateList", "getMassage", "getState"]),
  },
  async mounted() {
    let data = {
    ...
    this.selectedRegion = this.getStateList.find(item => item.id == this.getState.parent_id)
    feather.replace()
	this.form = this.getState;
  },
  methods: {
    ...mapActions("mystate", ["actionEditState", "actionStateList", "actionUpdateState", 'actionStates']),
    isRequired(input) {
